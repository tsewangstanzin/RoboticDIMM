...
class ImageGrab:
	...
	def single_image_high_gain(self):
		# print(f'Device used:\n\t{self.device}')
		...
		nodes['BinningVertical'].value = 3
		nodes['BinningHorizontal'].value = 3

		nodes['AcquisitionFrameRateEnable'].value = True
		# odes['AcquisitionFrameRate'].value = nodes['AcquisitionFrameRate'].max
		...
		with self.device.start_stream(1):
			...
			png_name = 'Main_Single_image_high_gain.png'
	...
	def create_cube(self,wd):
		...
		nodes['AcquisitionFrameRate'].value = 50.0
		'''
        If the frame rate is 30 FPS, the maximum allowable exposure would be 1/30 = 0.0333 seconds = 33.3 milliseconds.
        So, a decrease in the frame rate is necessary for increasing the exposure time.
        '''
		...
		filename2 = str(current_time.year) + str("_") + str(current_time.month) + str("_") + str(
			...
			"_") + str(current_time.second) + str("_") + str(current_time.microsecond) + ".fits"
		hdr22 = fits.Header()
		hdr22['EXP'] = (nodes['ExposureTime'].value, 'microsecond')
		hdr22['Gain'] = (nodes['Gain'].value, 'Gain Set')
		hdr22['Site'] = "IAO Hanle"
		...
		print("Number of fits file:", len(list_of_files))
		if(len(list_of_files)>=51):
			'''
			shutil.copy(list_of_files[0], 'fitscube')
			if (os.path.exists("/media/iiap/External_SSD_Disk/DIMM/"+str(wd))):
				print("External folder4 exist.......")
				os.system("mv *.fits /media/iiap/External_SSD_Disk/DIMM/" + str(wd))
			if not os.path.exists("/media/iiap/External_SSD_Disk/DIMM/"+str(wd)):
				os.mkdir("/media/iiap/External_SSD_Disk/DIMM/"+str(wd))
				os.system("mv *.fits /media/iiap/External_SSD_Disk/DIMM/"+str(wd))
			'''
			for file in list_of_files[:50]:
				os.remove(file)
			remaining_file=glob.glob('*.fits')
			shutil.move(remaining_file[0], 'fitscube')
		os.chdir('../')
...
def main():
	...
	i=0
	while (1):
		 grabber.single_image_high_gain()
		 print(
			 str("......Check:") + str(
				 i + 1), end="\r")
